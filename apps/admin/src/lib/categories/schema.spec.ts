...
describe('categorySchema', () => {
  ...
  it('accepts order 0 and a boolean isActive', () => {
    const r = categorySchema.safeParse({ name: 'Day Tours', order: 0, isActive: false });
    expect(r.success).toBe(true);
  });
});
...
describe('toCategoryPayload', () => {
  it('omits empty optionals and keeps set ones', () => {
    expect(
      toCategoryPayload({ name: 'Adventure', slug: '', description: 'Thrills', order: 2, isActive: true }),
    ).toEqual({ name: 'Adventure', description: 'Thrills', order: 2, isActive: true });
  });

  it('keeps order 0 (a valid explicit value)', () => {
    expect(toCategoryPayload({ name: 'Day Tours', order: 0 })).toEqual({ name: 'Day Tours', order: 0 });
  });
