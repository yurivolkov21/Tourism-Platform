...
describe('pickFeaturedDestinations', () => {
  it('returns the curated 4 in curated order, dropping the non-featured one', () => {
    ...
    expect(out.map((t) => t.slug)).toEqual(['ha-long-bay', 'sa-pa', 'ninh-binh', 'hanoi']);
  ...
  it('pads to 4 from the rest when a curated slug is missing', () => {
    ...
    expect(out.map((t) => t.slug)).toEqual(['ha-long-bay', 'sa-pa', 'ninh-binh', 'ha-giang']);
  });

  it('falls back to the first N (input order) for an unknown region', () => {
    const out = pickFeaturedDestinations(north, 'Other', 4);
    expect(out.map((t) => t.slug)).toEqual(['hanoi', 'ha-long-bay', 'ninh-binh', 'sa-pa']);
  });

  it('returns all when fewer than the limit are available', () => {
    const items = [tile('ha-long-bay'), tile('sa-pa')];
    expect(pickFeaturedDestinations(items, 'Northern Vietnam').map((t) => t.slug)).toEqual([
      'ha-long-bay',
      'sa-pa',
    ]);
  });
  ...
  it('does not mutate the input array', () => {
    ...
    expect(items.map((t) => t.slug)).toEqual(['hanoi', 'ha-long-bay', 'ninh-binh', 'sa-pa', 'ha-giang']);
  });
});
