/** Minimal shapes the metrics need (TourCardData / DestinationTileVM satisfy these). */
...
export interface AboutMetrics {
  ...
  destinations: number;
  regions: number;
  /** Review-count-weighted average rating (1-dp), or 0 when there are no reviews. */
  rating: number;
}

/** Real "by the numbers" figures for the About page, computed from the live catalog. */
