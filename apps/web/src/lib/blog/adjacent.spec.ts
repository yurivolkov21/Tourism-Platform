...
describe('pickAdjacentPosts', () => {
  ...
  it('has no newer neighbor for the newest post', () => {
    expect(pickAdjacentPosts(posts, 'newest')).toEqual({ newer: null, older: posts[1] });
  });

  it('has no older neighbor for the oldest post', () => {
    expect(pickAdjacentPosts(posts, 'oldest')).toEqual({ newer: posts[1], older: null });
  });

  it('returns nulls when the slug is not in the window', () => {
    expect(pickAdjacentPosts(posts, 'ghost')).toEqual({ newer: null, older: null });
  });
