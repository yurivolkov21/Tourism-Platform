...
describe('pageView', () => {
  it('derives total pages, clamped page and slice bounds', () => {
    expect(pageView(25, 1, 10)).toEqual({ totalPages: 3, page: 1, start: 1, end: 10 });
    expect(pageView(25, 2, 10)).toEqual({ totalPages: 3, page: 2, start: 11, end: 20 });
    expect(pageView(25, 3, 10)).toEqual({ totalPages: 3, page: 3, start: 21, end: 25 });
  });
  ...
  it('handles an empty set (1 page, zero bounds)', () => {
    expect(pageView(0, 1, 10)).toEqual({ totalPages: 1, page: 1, start: 0, end: 0 });
  });

  it('handles a partial single page', () => {
    expect(pageView(7, 1, 10)).toEqual({ totalPages: 1, page: 1, start: 1, end: 7 });
  });
});
...
describe('pageNumbers', () => {
  ...
  it('collapses both sides around the middle', () => {
    expect(pageNumbers(10, 5)).toEqual([1, 'ellipsis', 4, 5, 6, 'ellipsis', 10]);
  });
