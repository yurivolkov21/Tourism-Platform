    ...
    <section className="relative isolate overflow-hidden py-20 sm:py-28">
      ...
      <div className="text-primary-foreground mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="max-w-2xl space-y-4">
          ...
          <p className="text-primary-foreground/85 text-lg text-pretty">{body}</p>
        </div>

        <dl className="mt-12 grid grid-cols-2 gap-8 sm:mt-16 sm:grid-cols-4">
          {stats.map((s) => (
            <div key={s.label} className="border-primary-foreground/15 border-t pt-4">
              <dt className={cn('font-heading text-3xl font-bold sm:text-4xl', accentText)}>
                {s.value}
              </dt>
              <dd className="text-primary-foreground/75 mt-1 text-sm text-pretty">{s.label}</dd>
            </div>
          ))}
        </dl>
      </div>
