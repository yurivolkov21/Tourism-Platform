...
export function TourOverview({ tour }: { tour: TourDetailVM }) {
  ...
  const styles = (tour.travelStyles ?? [])
    .map((k) => styleLabels[k])
    .join(', ');
  ...
  const rows: { icon: ReactNode; label: string; value: string }[] = [
    ...
    },
    {
      ...
      label: s.departure,
      value: tour.departureFrequency,
    },
    ...(styles
      ? [
          {
            icon: <CompassIcon className="size-5" aria-hidden="true" />,
            label: s.travelStyle,
            value: styles,
          },
        ]
    ...
    ...(themes
      ...
      : []),
    {
      icon: <BedDoubleIcon className="size-5" aria-hidden="true" />,
