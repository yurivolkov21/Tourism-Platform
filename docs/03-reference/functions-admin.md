...
| A-TUR-4 | Update Tour<br>`PATCH /admin/tours/:slug` | 1. Admin sửa tour (publish/ẩn, giá, nội dung, `suitableFor`/`badges`…)<br>2. Gửi `destinationSlugs` thì revalidate + thay cả set M:N; slug gửi kèm qua `slugify()`<br>3. Cập nhật (sub-entity gửi kèm = replace-all)<br>4. Trả tour | Admin | **Tour**, Destination, … | tours, tour_destinations, … | Activity | ✅ **API-W2**: unpublish khi còn booking PAID/PARTIALLY_REFUNDED trên departure tương lai (kể cả hôm nay, mốc đầu-ngày-UTC) → 409 `TOUR_HAS_ACTIVE_BOOKINGS` |
...
| A-DEP-3 | Update Departure<br>`PATCH /admin/tours/:slug/departures/:id` | 1. Admin sửa departure (ghế, giá, trạng thái…)<br>2. Guard `seatsTotal >= seatsBooked` (400 `SEATS_TOTAL_BELOW_BOOKED`); sửa 1 ngày → validate chéo<br>3. Cập nhật; trả departure. **API-W2 — set `CANCELLED`** (cần user đã sync, 400 `USER_NOT_SYNCED`): giết booking PENDING → auto-refund tuần tự từng booking PAID qua `refundByAdmin` (email refund kèm Reason) → trả kèm `cancellation` summary `{paidTotal, refunded, skipped, failed}`; webhook đến muộn trên booking đã CANCELLED được refund capture tự động (outcome `cancelled` của `claimSeatsForPaid`) | Admin | **TourDeparture**, Booking, Outbox | tour_departures, bookings, outbox | Sequence | ✅ |
...
| A-REV-2 | Moderate Review<br>`PATCH /admin/reviews/:id/moderation` | 1. Admin xem review chờ duyệt<br>2. Gửi `{ isApproved: true \| false }`<br>3. 404 `REVIEW_NOT_FOUND` nếu thiếu<br>4. Trong `$transaction`: cập nhật `isApproved` **+ audit `moderated_by`/`moderated_at` (API-W3 — mỗi lần duyệt/từ chối, quyết định mới nhất thắng; cần user đã sync, 400 `USER_NOT_SYNCED`)**; **khi false→true** thì ghi `outbox` REVIEW_APPROVED (email — S-JOB-1, `skipDuplicates`)<br>5. Trả review; list admin kèm `moderatedBy {fullName,email}` | Admin | **Review**, User, Outbox | reviews, outbox | Activity | ✅ |
