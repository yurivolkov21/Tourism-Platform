...
| A-MED-2 | List Media Library<br>`GET /admin/media` | 1. Admin mở thư viện `/media` toàn hệ thống (hoặc dialog "Choose from library" trong `MediaField`, wave D1)<br>2. Lọc `ownerType`/`role`/`type` + search publicId + **`excludeUserOwned`** (ẩn avatar khách khỏi picker, wave D1), phân trang<br>3. Server resolve **owner** cho từng asset (tour/destination/post/user) để hiện link<br>4. Trả danh sách + meta; mỗi dòng kèm **`alt`** (nullable, wave D1) | Admin | **MediaAsset**, Tour, Destination, Post, User | media_assets, … | Activity | ✅ |
