...
| U-REV-5 | My Reviews<br>`GET /reviews/mine` (API-W3) | 1. User xem lại các review mình đã gửi<br>2. JWT bắt buộc (401 `USER_NOT_SYNCED` nếu chưa sync)<br>3. Server trả review của chính caller — **gồm cả chưa duyệt** (`isApproved` lộ trạng thái chờ), mới nhất trước, cap 50, kèm `tour {slug,title}`<br>4. Bổ trợ: `BookingDto.hasReview` trên `/bookings/me` + detail để web ẩn form review không cần dò 409 | Customer | **Review**, Tour | reviews, tours | Activity | ✅ |
