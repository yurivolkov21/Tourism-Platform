...
| **PayPal** (sandbox) | `PAYPAL_CLIENT_ID`, `PAYPAL_CLIENT_SECRET`, `PAYPAL_MODE=sandbox`, `PAYPAL_WEBHOOK_ID` | [developer.paypal.com](https://developer.paypal.com) → sandbox app. **Optional** — leave blank and the PayPal rail is simply unconfigured; Stripe still works. | No |
...
| **Resend** (email) | `RESEND_API_KEY`, `RESEND_FROM_EMAIL` | [resend.com](https://resend.com) → API key. ⚠️ **Required at boot** even if you don't care about email — the API refuses to start with it empty. Without a **verified domain** sends will fail: outbox rows go `FAILED` and retry. That's expected and harmless in dev (a placeholder-shaped `re_...` value boots fine). | **Yes** (non-empty) |
