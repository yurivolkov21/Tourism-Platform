...
- **English-only** ([ADR-0005](../02-decisions/0005-en-only.md)); copy centralized in `@tourism/i18n` (EN scaffold).
...
`@nx/enforce-module-boundaries`). Full rule table: [architecture](../01-architecture/README.md#2-module-boundaries)
and [BLUEPRINT §3](../BLUEPRINT.md). TL;DR: `scope:shared` → only `scope:shared`; apps can't import apps.

## 4. i18n (English-only)

EN-only ([ADR-0005](../02-decisions/0005-en-only.md)). User-facing copy is
...
Hardened beyond the donor — see [ADR-0008](../02-decisions/0008-security-integrity-hardening.md)
and [risks.md](../01-architecture/risks.md). Baseline every feature respects:
