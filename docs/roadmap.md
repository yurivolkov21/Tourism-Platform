...
| **P3** | Web (customer): home → destinations → tours → detail → booking → account | 🚧 **~90%** — home · destinations (overview + **3 region pages**) · tours (listing **+ free-text search** · detail) · about · contact (**real enquiry + interest dropdown from live categories**) · content pages · **auth (login/register/forgot/reset)** · **account (dashboard · settings · bookings)** · **booking flow (Stripe/PayPal + private-departure request + checkout pages)** all done; **component reform done** (forms → `@tourism/ui`). **Remaining:** region-detail real data · wishlist save-UI · motion/a11y/perf pass (see P3 breakdown) |
...
| **DEPLOY** | Free-tier cloud deploy (Vercel + Render + Supabase) | ✅ **LIVE** — web+admin on **Vercel**, API (NestJS+pg-boss) on **Render** free, DB on **Supabase**; `/health` (DB ping) + GH-Actions keep-alive pinger + Cloudflare-tunnel fallback. Runbook: [deploy](05-runbooks/deploy.md). Upgrade-to-paid path = no code change. |
...
| **Tours listing** | `/tours` | ✅ static — sidebar facets (Destination · Category · Duration · Price) + mobile drawer · sort · `TourCard` grid · empty state · **free-text search** (`searchTours` in `@tourism/core`, accent/đ-insensitive; hero `?q=` feeds it). **Client-side filter** (`filterTours`/`sortTours`) keeps the page static. |
...
| **Wire real data** | — | 🚧 **mostly wired** — home featured/bento · `/destinations` overview · `/tours` listing+detail · enquiry → `POST /enquiries` · reviews (tour-detail + homepage testimonials) · contact interest dropdown (all via `@tourism/core` / native fetch, ISR). **Remaining:** region-detail `/destinations/[region]` still fixtures. |
