...
| **P4** | Admin: manage tours/destinations/departures/media/reviews/bookings | ✅ **CRUD breadth done + DEPLOYED** — admin auth (Supabase SSR + proxy, `/auth/admin/sync` allowlist) · app shell (sidebar/topbar/theme/user-menu) · dashboard (live `/admin/stats/dashboard`) · **CRUD: Destinations · Categories · Tours · Departures · Posts** (Server-Component fetch + Server-Action mutations, `@tourism/ui`, token-only). Dev port **:3002**. **UI REDESIGN in progress (2026-06-30):** Login (React Bits Aurora WebGL backdrop + glass card + shared Nexora logo) · app shell (inset sidebar, dashboard-01 pattern, footer account) · **dashboard rebuilt to shadcn dashboard-01 parity** (SectionCards + interactive daily area chart 90/30/7d + TanStack/dnd DataTable on real bookings; BE `dailyTrend` added) — all DONE + deployed. Remaining: reskin catalog CRUD pages · Tours increment-2 (itinerary/FAQs/policies) · **build missing modules: Bookings (list/detail/refund) · Enquiries/CRM · Media upload**. |
