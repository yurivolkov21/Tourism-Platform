...
| Route | Render | Notes |
| --- | --- | --- |
| `/` | static | Lily-style homepage (hero · destinations bento · experiences · featured · why-choose · trust · blog-teaser · enquiry). **Trust band (2026-07-17): real data only** — prop-fed `TrustStats` (live review average + published-tour count via `buildTrustSectionStats`); no-data rows hidden, grid rebalances (`trustGridClass`); 24/7-support is the one static pledge. Testimonials (`/destinations`) render only real featured reviews — none ⇒ hidden (no fixture fallback). |
...
| `/tours/[slug]` | **SSG + ISR 300** | `generateStaticParams` from live tour slugs (`fetchTourDetailSlugs`); unknown → `notFound()`. Tour hero · overview · highlights · **itinerary accordion** · what's-included · sticky **BookingBox** (real "Book now" → `/tours/[slug]/book` · "Travel on your own dates" → private request · **seats-left per departure** · wishlist heart for signed-in users) · gallery · enquiry. Reviews + detail fetches are **tagged `tour:<slug>`** (`lib/revalidate.ts`) so the API busts them on review moderation (see the revalidation route below). **Reviews section (2026-07-17)**: 5-line clamp + max 6 inline cards (layout-stable); measured "Read more" → full-review Dialog; "See all {N}" Dialog pages `GET /tours/:slug/reviews` browser-side (id-deduped, manual retry); server shell + `ReviewCard`/`SeeAllReviews` client islands, pure logic in `lib/reviews-pager.ts`. |
| `/api/revalidate` | **Route Handler** (POST, `force-dynamic`) | **Generalized on-demand cache busting** ([ADR-0013](../02-decisions/0013-api-driven-web-revalidation.md)). Body `{ tags?, paths? }` (legacy `{ slug }` kept); STRICT allow-list via the tag taxonomy in `lib/revalidate.ts` (`site-media`/`tours`/`tour:<slug>`/`destinations`/`posts`/`post:<slug>`/`categories`/`featured-reviews`/`trust-stats`) — any unknown entry → 400, nothing busted. Secret-guarded (`REVALIDATE_SECRET`, constant-time): 503 unset · 401 bad. `revalidateTag(tag, { expire: 0 })`/`revalidatePath`. Called post-commit by the NestJS API from every content-mutating module; **every public fetch is tagged** and the ISR timers (≤300s; `/blog` now included) are just the backstop. |
...
| `/login` `/register` `/forgot-password` `/reset-password` · `/account/*` | dynamic | Supabase auth + account hub (dashboard · settings · bookings + detail/cancel · saved). Booking detail carries the **"Rate this trip" review form** on PAID bookings (wave W1, 2026-07-12): star radiogroup + BE-limit validation, 409 → "already reviewed" panel, success → awaiting-moderation panel. **Change-email (2026-07-16)**: gated to password-only accounts (`canChangeEmail` over `app_metadata.providers`; Google-linked → managed-by-Google note), current-password re-auth (`signInWithPassword`) before `updateUser` (pairs with Supabase Secure-email-change OFF). |
| `/auth/callback` · `/auth/confirm` | **Route Handlers** | Auth confirmation. `/auth/callback` = **PKCE** `exchangeCodeForSession` (OAuth). `/auth/confirm` (2026-07-16) = **token_hash** `verifyOtp({type, token_hash})` for the email-link flows (`signup`/`recovery`/`email_change`/`invite`) — verifies server-side so confirmation completes **cross-browser** (no PKCE verifier); on success `syncUser()` refreshes the API email mirror, then `safeRedirect` (token stripped), failure → `/login?error=auth`. Type guard + parse in `lib/auth/confirm-params.ts`. The 3 Supabase templates (`docs/email-templates/`) link here via `{{ .TokenHash }}`. |
