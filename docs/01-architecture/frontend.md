...
| `/api/revalidate` | **Route Handler** (POST, `force-dynamic`) | On-demand ISR busting. Secret-guarded by the shared `REVALIDATE_SECRET` (`isValidRevalidateSecret`, constant-time): 503 unset · 401 bad/missing header · 400 no slug · 200 → `revalidateTag('tour:<slug>', { expire: 0 })`. Called by the NestJS API after a review is (un)approved so the tour page reflects it on the next reload without going dynamic. |
