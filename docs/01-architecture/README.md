...
- **scope:** `shared`→shared · `api`→api,shared · `web`→web,shared · `mobile`→mobile,shared · `admin`→admin,**web**,shared ([ADR-0004](../02-decisions/0004-admin-reuses-web-ui.md))
...
- **Payments = Stripe + PayPal** (multi-gateway, [ADR-0006](../02-decisions/0006-multi-gateway-momo.md) — amended MoMo→PayPal; audience is inbound foreign tourists). Stripe (hosted Checkout) + PayPal (Orders v2, capture-on-return). Each: own webhook (signature verify), shared `PaymentEvent` idempotency + **atomic seat-claim CTE** (no `FOR UPDATE`). Stripe ported from donor.
...
## 6. Security & integrity ([ADR-0008](../02-decisions/0008-security-integrity-hardening.md))
...
## 7. Reliability ([ADR-0007](../02-decisions/0007-pgboss-outbox-jobs.md))
