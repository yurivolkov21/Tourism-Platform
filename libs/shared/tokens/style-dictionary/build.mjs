...
// React Native theme stub — shape validation for the future mobile app.
// NOTE: colors are oklch strings; convert to RN-compatible values when mobile is built.
StyleDictionary.registerFormat({
  ...
  format: ({ dictionary }) => {
    ...
    const entries = (pick) =>
      colors.map((t) => `      '${key(t)}': '${pick(t)}',`).join('\n');
