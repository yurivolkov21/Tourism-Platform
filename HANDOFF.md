...
> detection) + **C (booking-detail price breakdown + tab counts · post SEO
> fields + scheduled publishing · admin self-profile · subscriber remove ·
> outbox delete · `/payment-events` viewer) are DONE** — next up is **wave
> D**: dashboard analytics/date-range · real notifications · media-library
> reuse/picker + bulk delete (+ row selection) · category imagery ·
> `TabPills` extraction (the tablist pill markup repeats in 6+ files) ·
> last-admin-demote race · admin e2e — pick with the user when ready.
