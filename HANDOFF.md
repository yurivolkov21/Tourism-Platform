...
> **Admin debt processing (user-approved 2026-07-11): waves B1 → B2 → C →
> D1, then D2.** B1 (list-tables) + B2 (reviews edit-curated · user/booking
> links · rating filter · server pagination + enquiry internal notes +
> repeat-lead detection) + C (booking-detail price breakdown + tab counts ·
> post SEO fields + scheduled publishing · admin self-profile · subscriber
> remove · outbox delete · `/payment-events` viewer) + **D1 (media library
> upgrade — reuse picker · ref-safe GC · editable alt · bulk delete) are
> DONE** — next up is **wave D2**: `TabPills` extraction (the tablist pill
> markup repeats in 6+ files) · dashboard custom date-range + Top Tours
> multi-currency · last-admin-demote race fix. (Still cut: notifications ·
> category imagery · admin e2e.)
