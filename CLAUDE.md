...
| `@tourism/mobile` | `apps/mobile` | Expo SDK 54 / RN | `scope:mobile,type:app` | 🚧 **P5 W1 + W2 DONE (W2 merged 2026-07-07)** — expo-router 4-tab shell (Nexora-themed) · env-switchable `@tourism/core` client + TanStack Query · **Home w/ real featured tours** · **W2 browse & detail**: real Explore tab (instant client-side search/filter/sort via core helpers · destination chips rail · duration/price/sort chips · 4 data states) · tour-detail stack screen at web parity (gallery pager · seats-left · itinerary/FAQ accordions · reviews · sticky Inquire CTA) · enquiry bottom-modal (validated, 429-aware) · **three-layer background theming** (Stack `contentStyle` + Tabs `sceneStyle` + nav `ThemeProvider` + `expo-system-ui` root view — kills the transition white-flash) · device-polish audit (pressed states/hitSlop · keyboard-friendly taps · themed RefreshControl) · jest `testTimeout: 20000` (RN suites blow Jest's 5s on the shared CI runner). All verified on-device (Expo Go, Android). Dev loop: `pnpm exec expo start` from `apps/mobile` (via nx = non-interactive, no QR); react pinned **19.1.0** workspace-wide (SDK 54). **33 tests**. Next: **W3 auth/account**. |
...
| `@tourism/mobile-ui` | `libs/mobile/ui` | mobile design system (RN) | `scope:mobile,type:ui` | 🟢 **founded (P5 W1) + W2 additions** — ThemeProvider/`useTheme` over `@tourism/tokens/theme` (RN hex output, follows OS dark mode) + AppText · Screen (hides scroll indicators by default) · Spinner · Button · Card · **TextField · Chip · Accordion**. 26 tests. |
