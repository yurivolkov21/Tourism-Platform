...
| `@tourism/web` | `apps/web` | Next.js 16 · React | `scope:web,type:app` | 🟢 **P3 in progress** — **brand "Nexora"** (NEX origami logo + favicon, set in `@tourism/i18n`) · **redesigned navbar** (scroll→glass-pill, hover-pill links, animated arrow CTA) + home **"Built with" tech-cloud** marquee · home · destinations (overview + **3 region L2 pages** + enquiry) · `/faq` `/privacy` `/terms` · about · **contact (Contact-01 form + MapLibre/mapcn map + FAQ)** · **tours (listing + detail)**. **Real data WIRED** (home featured/bento · `/destinations` · `/tours` · `/tours/[slug]` · enquiry→`POST /enquiries`, ISR 300). **Next:** booking flow + customer auth, region-detail real data |
