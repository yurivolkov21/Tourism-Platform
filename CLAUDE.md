...
| `@tourism/web` | `apps/web` | Next.js 16 · React | `scope:web,type:app` | 🟢 **Complete + deployed (Vercel)** — brand **"Nexora"**; all surfaces on real data (home · destinations + 3 region pages · tours + detail · booking money-path w/ private requests · auth/account · blog v2 · contact/FAQ/legal); a11y WCAG 2.2 AA + SEO + perf polish done; feedback layer (toast/flash + `AlertDialog` confirms); every form `noValidate` + per-field server errors (standing rule); brand chrome admin-managed via site-media w/ per-slot fallbacks; renders prefer `media.alt`. **232 tests.** Map: [frontend.md](docs/01-architecture/frontend.md) · history: [CHANGELOG](docs/CHANGELOG.md). |
